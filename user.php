<?php
// Borrow and Return Functions only
...
$message = '';
$error = '';
// Initialize search variable, safely grabbing from GET
$search_query = $_GET['search'] ?? ''; 

// =========================================================================
// 2. HANDLE POST REQUESTS (BORROW & RETURN)
// =========================================================================

// --- A. Handle RETURN Action ---
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['return_action'])) {
    ...
    try {
        // 1. Mark as returned in borrowed_books
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['borrow_action'])) {
    ...
    try {
        // 1. Decrease available copies (must be greater than 0)
...
// =========================================================================
// 3. FETCH DATA FOR DISPLAY (MODIFIED FOR SEARCH)
// =========================================================================
...
if (!empty($search_query)) {
    // Sanitize search query and wrap with wildcards
...
// Use prepared statement if parameters are needed (i.e., when searching)
if (!empty($params)) {
    if ($stmt_available = $conn->prepare($sql_available)) {
        ...
        $result_available = false; // Prevent display if prepare failed
    }
} else {
    // Standard query if no search term
    $result_available = $conn->query($sql_available);
}


// --- Fetch Borrowed Books (for Returning) ---
$books_borrowed = [];
...
<head>
    ...
    <style>
        ...
        /* Centered title class for main heading */
        .centered-title { text-align: center; }

        /* Navigation Links (Buttons) - Now only for Logout */
        .nav-links a { 
        ...
        /* Form Buttons (Borrow and Return) */
        .action-button {
            background-color: var(--green-primary); 
            color: white; 
            border: none; 
...
<div class="container">
    ...
    <?php 
    // Check if the result set is valid before iterating
